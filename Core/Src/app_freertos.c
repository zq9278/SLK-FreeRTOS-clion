  ...
  /* USER CODE BEGIN AppMotor_Task */
    ...
    for (;;)
    {
        ...
        else if ((((Motor_Event_Bit & Motor_BIT_2) != 0 || (Motor_Event_Bit & Auto_BIT_3)) != 0) && ((Motor_Event_Bit & SW_BIT_1) != 0)) // 脉动或�?�自动事件发生，按钮事件发生（正式脉动模式启动）
        {
            ...
            {
                case 1:
                    // if (ForceRawActual < (ForceRawOffset + ForceRawSet))
                    // {
                    //   MotorCompareState = 1;
                    //   break;
                    // }
            ...
            //Limit(Force_Q,0,ForceRawOffset + ForceRawSet-102672);
            Limit(Force_Q,0,ForceRawSet+82617);
            xQueueSend(Force_QueueHandle, &Force_Q, 0);
        }
            //     else if ((Motor_Event_Bit & Motor_BIT_2) == 0)  // 脉动或�?�自动事件发生，按钮事件发生（正式脉动模式启动）|| ((Motor_Event_Bit & Auto_BIT_3) == 0)) && ((Motor_Event_Bit & SW_BIT_1) == 0)
            //     {      vTaskDelay(10000 );
